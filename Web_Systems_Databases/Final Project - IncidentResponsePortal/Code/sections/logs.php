...
<article id="logs_body_container">
    ...
    <div>
        <ul class="logs_pagination_container">
            <?php 
                $paginationVisible = 7;
                $half = floor($paginationVisible / 2);

                $start = max(1, $currentPage - $half);
                $end = min($totalPages, $currentPage + $half);

                // adjust in the beginning or ending
                if($end - $start + 1 < $paginationVisible){
                    if($start == 1){
                        $end = min($totalPages, $start + $paginationVisible - 1);
                    } else if($end == $totalPages){
                        $start = max(1, $end - $paginationVisible + 1);
                    }
                }

                if($start > 1): ?>
                    <li class="log_pagination_li">...</li>
                <?php endif; ?>
